...
Public Module modGeneral
    ...
    Public Function FormTeachers(filter As String) As List(Of ComboItem)
        ...
        Dim sql As String = "spLoadFormTeachers"
        cnn = New SqlConnection(ConnectionString)
        cmd = New SqlCommand(sql, cnn)
        Try
            cnn.Open()
            ...
            While drr.Read
                itm = New ComboItem(drr("FullName"), drr("Teacher"))
                values.Add(itm)
            End While
        Catch ex As Exception
        Finally
    ...
    Public Function Sportsmasters() As List(Of ComboItem)
        Dim itm As ComboItem
        Dim values As New List(Of ComboItem)
        Dim drr As SqlDataReader

        Dim sql As String = "spLoadSportsmasters"
        cnn = New SqlConnection(ConnectionString)
        cmd = New SqlCommand(sql, cnn)
        cnn.Open()
        cmd.CommandType = CommandType.StoredProcedure
