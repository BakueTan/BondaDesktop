...
" order by c.clas desc,c.sem desc"
    ...
    Public Sub LoadSchoolFeesPeriod()
        Dim PeriodsList As New List(Of schoolFeesPeriods)
        Dim operiod As schoolFeesPeriods

        Dim periods As SqlDataReader

        sql = "spLoadProfileFeesPeriods"
        ...
        Try
            ...
            periods = cmd.ExecuteReader()

            While periods.Read
                operiod = New schoolFeesPeriods
                If PERIODCOUNTER = 0 Then
                    Period = periods("period")
                    PeriodDesc = periods("Description")
                    operiod.Period = Period
                    operiod.PeriodDescription = PeriodDesc
                    PeriodsList.Add(operiod)
                    '    LoadAcademicReport()
                End If

                lbStudPeriods.DataSource = PeriodsList
                lbStudPeriods.DisplayMember = "PeriodDescription"
                lbStudPeriods.ValueMember = "Period"
    ...
    Private Sub LoadAcademicReport()
        ...
        ElseIf program.ToUpper = "A_LEVEL" Then
            ReportViewer1.LocalReport.ReportEmbeddedResource = "StudentPersonalDetails.rptHsReport.rdlc"
        End If
