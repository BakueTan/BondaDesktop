...
Public Class frmStaffDetails
    ...
    Private Quals As List(Of cStaffQualification)
    Public Sub New()
        ...
        Try
            ...
            With cmd
                ...
                If StaffIDTextBox.Text = "" Then
                    ...
                    If Not IsNothing(Quals) Then
                        sqltext = "spInsertStaffQualification"



                        For Each staffqual As cStaffQualification In Quals
                            cmd = New SqlCommand(sqltext, cnn, trans)
                            cmd.CommandType = CommandType.StoredProcedure
                            With staffqual


                                cmd.Parameters.AddWithValue("@staff", SavedStaff)
                                cmd.Parameters.AddWithValue("@qual", .Qualification)
                                cmd.Parameters.AddWithValue("@instutution", .Institution)
                                cmd.Parameters.AddWithValue("@year", .Period)
                                cmd.ExecuteNonQuery()
                            End With
    ...
    Private Sub Button9_Click(sender As Object, e As EventArgs) Handles btnAddStaffQual.Click
        Dim parameter As SqlParameter = Nothing
        Dim parameters As List(Of SqlParameter) = Nothing
        Dim staffqual As New cStaffQualification
        Dim errormsg As String = ""

        cnn = New SqlConnection(ConnectionString)


        If IsNothing(Quals) Then
            Quals = New List(Of cStaffQualification)
        End If

        With staffqual
            .StaffID = StaffIDTextBox.Text
            .Qualification = cboStaffQualQualification.Text
            .Period = txtStaffQualYear.Text
            .Institution = txtQualificationInstitution.Text
        End With




        If Trim(StaffIDTextBox.Text) <> "" Then ' AddTodartabase
            Try
                cnn.Open()
                ...
                With staffqual
                    parameter = New SqlParameter("@staff", .StaffID)
                    parameters.Add(parameter)
                    parameter = New SqlParameter("@qual", .Qualification)
                    parameters.Add(parameter)
                    parameter = New SqlParameter("@instutution", .Institution)
                    parameters.Add(parameter)
                    parameter = New SqlParameter("@year", .Period)
                    parameters.Add(parameter)
                    ...
                    Next
                    cmd.ExecuteNonQuery()
                    MsgBox("Staff Qualification Successfully added")
                End With
            Catch ex As Exception
                MsgBox(ex.Message)
            Finally
            ...
            End Try
            LoadStaffQual(StaffIDTextBox.Text)

        Else 'Add to Grid

            Quals.Add(staffqual)
            dgStaffQual.DataSource = Nothing
            dgStaffQual.DataSource = Quals
    ...
    Private Sub LoadStaffQual(Staff As String)
        ...
        Dim sql As String
        Dim cnn As New SqlConnection(ConnectionString)
        sql = " select * from fnLoadStaffQual(@staff)"

        Try
            Dim cmd As New SqlCommand(sql, cnn)
            cnn.Open()
            param = New SqlParameter("@staff", Staff)
            cmd.Parameters.Add(param)
            taApps = New SqlDataAdapter(cmd)
            taApps.Fill(dsApps)
            dgStaffQual.DataSource = dsApps.Tables(0)
        Catch ex As Exception
            MsgBox(ex.Message)
        Finally
            cnn.Close()
    ...
    Private Sub StaffIDTextBox_TextChanged(sender As Object, e As EventArgs) Handles StaffIDTextBox.TextChanged
        dgStaffQual.DataSource = Nothing
        Quals = New List(Of cStaffQualification)
        LoadStaffQual(StaffIDTextBox.Text)
    ...
    Private Sub dgStaffQual_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgStaffQual.CellContentClick

    End Sub

    Private Sub btnDeleteQualification_Click(sender As Object, e As EventArgs) Handles btnDeleteQualification.Click
        If StaffIDTextBox.Text <> "" Then

            If dgStaffQual.SelectedRows.Count = 0 Then
                MsgBox("Select at least One Record")
                Exit Sub

            End If
            Dim cnn As New SqlConnection(ConnectionString)
            Dim sql As String = "spDelStaffQuals"
            Dim ref As String = dgStaffQual.SelectedRows(0).Cells("RefID").Value.ToString
        ...
        Else
            If Quals.Any Then
                ...
                Quals.RemoveAt(dgStaffQual.SelectedRows(0).Index)
                dgStaffQual.DataSource = Nothing
                dgStaffQual.DataSource = Quals
